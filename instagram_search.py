...
class HashTagSearch(metaclass=ABCMeta):
    ...
    def extract_recent_tag(self, tag):
        ...
        f.write(response.get_text())
        f.close()

        potential_query_ids = self.get_query_id(response)
        shared_data = self.extract_shared_data(response)
        print('##################################')
        print(shared_data['entry_data']['TagPage'][0]['graphql']['hashtag'].keys())
        # print(shared_data['entry_data']['TagPage'][0]['graphql']['hashtag'])

        for i in shared_data['entry_data']['TagPage'][0]['graphql']['hashtag'].keys():
            print(i)

        print(shared_data['entry_data']['TagPage'][0]['graphql']['hashtag']['edge_hashtag_to_media'].keys())
